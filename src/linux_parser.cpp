...
float LinuxParser::MemoryUtilization() {
  // For memory utilization use: MemTotal - UseMem where
  // freeMem = MemFree + Buffers + Cached
  string key, kbString;
  float MemTotal{1};
  //float UseMem{0};
  float MemFree{0};
  ...
  if (stream.is_open()) {
    while( stream >> value )
    {
      ...
      if (value == "MemTotal:") {
        stream >> str_temp;
        MemTotal = std::stol(str_temp);
      } else if (value == "MemFree:") {
        stream >> str_temp;
        MemFree = std::stol(str_temp);
      } else if (value == "Buffers:") {
        stream >> str_temp;
        Buffers = std::stol(str_temp);
      } else if (value == "Cached:") {
        stream >> str_temp;
        Cached = std::stol(str_temp);
      }
  ...
  float UseMem = MemFree + Buffers + Cached;
  float memory_utilization = (MemTotal -  UseMem) / MemTotal;
...
long LinuxParser::ActiveJiffies(int pid) {
  ...
  string line, value;
  // Store elements of the file in a vector of strings, we'll convert the
  // elements we want
...
long LinuxParser::ActiveJiffies() {
  ...
  long activeJiffies = stol(totalJiffies[CPUStates::kUser_]) +
                       stol(totalJiffies[CPUStates::kNice_]) +
                       stol(totalJiffies[CPUStates::kSystem_]) +
                       stol(totalJiffies[CPUStates::kIRQ_]) +
...
long LinuxParser::IdleJiffies() {
  ...
  long idleJiffies = stol(totalJiffies[CPUStates::kIdle_]) +
                     stol(totalJiffies[CPUStates::kIOwait_]) +
                     stol(totalJiffies[CPUStates::kSteal_]);

  return idleJiffies;
}

// This is a function creates a vector of strings that contain the values of all
// jiffies
...
long LinuxParser::UpTime(int pid) {
  ...
  // This is where it gets a little tricky, the starttime is "the time the
  // process started after system boot" and not uptime directly. So grab the 
  // system uptime and subtract
  // starttime to get pid uptime.
  long systemUptime = UpTime();  // This is already in seconds
  long startTime =
      stol(values[21]) / sysconf(_SC_CLK_TCK);  // see proc man page
